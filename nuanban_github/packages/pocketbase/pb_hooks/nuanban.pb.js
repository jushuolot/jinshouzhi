...
routerAdd("POST", "/api/nuanban/dev-login", function (e) {
  ...
  if (users.length === 0) {
    return e.json(404, { message: "该邮箱账号不存在，请改用手机验证码登录并注册" });
  }
  const user = users[0];
  ...
  for (let i = 0; i < roleRecords.length; i++) {
    ...
    roles.push({
      ...
      status: nb.roleStatusForClient(r),
...
routerAdd("POST", "/api/nuanban/phone-login", function (e) {
  ...
  const email = nb.phoneLoginEmail(phone);
...
routerAdd("POST", "/api/nuanban/auth/register", function (e) {
  ...
  if (existing.length > 0) {
    if (role === "student") {
      var roleRec = existing[0];
      if (body.displayName) roleRec.set("display_name", body.displayName);
      if (body.contactPhone) roleRec.set("contact_phone", String(body.contactPhone).trim().slice(0, 20));
      else if (body.wechatId) roleRec.set("contact_phone", String(body.wechatId).trim().slice(0, 20));
      if (body.studentId != null) roleRec.set("student_id", String(body.studentId || "").trim().slice(0, 32));
      if (body.gender) roleRec.set("gender", String(body.gender));
      if (body.cartoonAvatarId) roleRec.set("cartoon_avatar_id", String(body.cartoonAvatarId));
      if (body.major != null) roleRec.set("major", String(body.major || "").trim());
      if (body.grade != null) roleRec.set("grade", String(body.grade || "").trim());
      if (body.serviceAreaPolygons != null) {
        nb.writeServiceAreaGeo(roleRec, { polygons: body.serviceAreaPolygons });
      }
      if (body.serviceHours != null) {
        nb.writeJsonStringArray(roleRec, "available_hours", body.serviceHours);
      }
      if (body.schoolName) {
        var snUpd = String(body.schoolName).trim();
        if (!nb.isKnownSchool(snUpd)) {
  ...
  if (body.contactPhone) rec.set("contact_phone", String(body.contactPhone).trim().slice(0, 20));
  else if (body.wechatId) rec.set("contact_phone", String(body.wechatId).trim().slice(0, 20));
  if (body.studentId != null) rec.set("student_id", String(body.studentId || "").trim().slice(0, 32));
  if (body.gender) rec.set("gender", String(body.gender));
  if (body.cartoonAvatarId) rec.set("cartoon_avatar_id", String(body.cartoonAvatarId));
  if (body.major != null) rec.set("major", String(body.major || "").trim());
  if (body.grade != null) rec.set("grade", String(body.grade || "").trim());
  if (body.serviceAreaPolygons != null) {
    nb.writeServiceAreaGeo(rec, { polygons: body.serviceAreaPolygons });
  }
  if (body.serviceHours != null) {
    nb.writeJsonStringArray(rec, "available_hours", body.serviceHours);
  }
  if (body.schoolName) {
    ...
    var schoolRec = nb.findOrCreateSchoolByName(schoolName);
    rec.set("school", schoolRec.id);
  }
  $app.save(rec);
  if (role === "elder") {
    nb.ensureElderProfileLinked(rec, auth, {
      displayName: body.displayName || auth.getString("name"),
      gender: body.gender,
    });
  }
  if (body.displayName) {
...
routerAdd("GET", "/api/nuanban/elder/caregivers/nearby", function (e) {
  ...
  const result = nb.listCaregiversForElder({
    lat: q.get("lat") || "0",
    lng: q.get("lng") || "0",
...
routerAdd("GET", "/api/nuanban/elder/caregivers/{id}", function (e) {
  ...
  const q = e.request.url.query();
  const elderLat = parseFloat(q.get("lat") || "0");
  const elderLng = parseFloat(q.get("lng") || "0");
  let roleRec = null;
  try {
  ...
  if (!nb.studentCanServeCaregiver(roleRec)) {
    return e.json(404, { message: "陪护同学不存在" });
  }
  return e.json(200, nb.caregiverDetailFromRole(roleRec, elderLat, elderLng));
});
...
routerAdd("POST", "/api/nuanban/elder/orders", function (e) {
  ...
  var familyUser = nb.primaryFamilyUserForElder(elderId);
  if (familyUser) order.set("family_user", familyUser);

  if (body.studentId) {
    var studentUserId = nb.resolveStudentUserId(body.studentId);
    if (!studentUserId) {
      return e.json(400, { message: "指定同学无效，请重新选择陪护同学" });
    }
  ...
  try {
    ...
    return e.json(400, { message: "创建订单失败：" + String(err.message || err) });
  }
  ...
  if (needsOutdoor && status !== "pending_payment") {
    ...
    try {
      ...
      if (familyUser) out.set("family_user", familyUser);
      $app.save(out);
    } catch (err) {
      return e.json(400, { message: "外出审批单创建失败：" + String(err.message || err) });
    }
  }
...
routerAdd("GET", "/api/nuanban/schools", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  var seen = {};
  var list = [];
  var i;
  for (i = 0; i < nb.KNOWN_SCHOOLS.length; i++) {
    var kn = nb.KNOWN_SCHOOLS[i];
    if (!seen[kn]) {
      seen[kn] = true;
      list.push(kn);
    }
  ...
  try {
    var recs = $app.findRecordsByFilter("school_dict", "", "name", 200, 0);
    for (i = 0; i < recs.length; i++) {
      var sn = nb.safeRecordString(recs[i], "name", "");
      if (sn && !seen[sn]) {
        seen[sn] = true;
        list.push(sn);
      }
...
routerAdd("PATCH", "/api/nuanban/student/profile", function (e) {
  ...
  if (body.wechatId != null) {
    roleRec.set("contact_phone", String(body.wechatId || "").trim());
  }
  if (body.contactPhone != null) {
    roleRec.set("contact_phone", String(body.contactPhone || "").trim());
  }
  if (body.studentId != null) {
    roleRec.set("student_id", String(body.studentId || "").trim());
  }
  if (body.serviceAreaPolygons != null) {
    nb.writeServiceAreaGeo(roleRec, { polygons: body.serviceAreaPolygons });
  }
...
routerAdd("POST", "/api/nuanban/student/cartoon-avatar", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  const rc = nb.assertActiveRoleHeader(e, "student");
  if (!rc.ok) return e.json(rc.code, rc.body);
  const auth = e.auth;
  ...
  const files = e.findUploadedFiles("avatar");
  if (!files || files.length === 0 || !files[0]) {
    return e.json(400, { message: "请上传头像图片" });
  }
  roleRec.set("custom_cartoon_avatar", files[0]);
  roleRec.set("cartoon_avatar_id", "");
  $app.save(roleRec);
  return e.json(200, {
    ok: true,
    customCartoonAvatarUrl: nb.roleFileUrlForClient(roleRec, "custom_cartoon_avatar", e),
  });
});
...
routerAdd("GET", "/api/nuanban/elder/profile", function (e) {
  ...
  if (!elder && roles.length > 0) {
    elder = nb.ensureElderProfileLinked(roles[0], auth, {});
  }
  if (!elder) return e.json(404, { message: "老人档案不存在，请重新选择身份注册" });
  let orgName = "";
  const orgId = elder.getString("org");
  if (orgId) {
    try {
  ...
  var orgComplete = nb.elderOrgProfileComplete(elder);
  return e.json(200, {
    id: elder.id,
    name: elderName,
    ...
    orgProfileHint: nb.elderOrgProfileHint(orgComplete),
    age: elder.getInt("age") || 0,
    gender: elder.getString("gender") || "",
...
routerAdd("GET", "/api/nuanban/family/profile", function (e) {
  ...
  const auth = e.auth;
  if (!auth) return e.json(401, { message: "需要登录" });
  var roleRec = nb.familyRoleRecord(auth.id);
  if (!roleRec) return e.json(404, { message: "家属角色不存在" });
  return e.json(200, nb.familyProfileDtoFromRole(roleRec, auth, e));
});
...
routerAdd("PATCH", "/api/nuanban/family/profile", function (e) {
  ...
  if (body.nickname) {
    ...
    auth.set("name", nick);
    roleRec.set("display_name", nick);
    $app.save(auth);
  }
  if (body.contactPhone != null) {
    roleRec.set("contact_phone", String(body.contactPhone || "").trim().slice(0, 20));
  }
  var meta = nb.readFamilyProfileMeta(roleRec);
  if (body.district != null) meta.district = String(body.district || "").trim();
  if (body.address != null) meta.address = String(body.address || "").trim();
  if (body.relationToElder != null) meta.relationToElder = String(body.relationToElder || "家属").trim();
  nb.writeFamilyProfileMeta(roleRec, meta);
  $app.save(roleRec);
  if (body.relationToElder) {
    ...
    for (var bi = 0; bi < bindings.length; bi++) {
      bindings[bi].set("relation_label", meta.relationToElder);
      $app.save(bindings[bi]);
    }
  }
  var dto = nb.familyProfileDtoFromRole(roleRec, auth, e);
  dto.ok = true;
  return e.json(200, dto);
});
...
routerAdd("POST", "/api/nuanban/org/orders/{id}/dispatch", function (e) {
  ...
  let studentUserId = nb.resolveStudentUserId(body.studentUserId || "");
  if (!studentUserId) {
    return e.json(400, { message: "请指定有效学生" });
  }
  var studentRoles = $app.findRecordsByFilter(
    "user_roles",
    'user = {:uid} && role = "student" && status = "active"',
    "",
    1,
    0,
    { uid: studentUserId }
  ...
  if (studentRoles.length === 0) {
    return e.json(400, { message: "该学生未通过审核或不存在" });
  }
  order.set("student_user", studentUserId);
  order.set("status", "pending_service");
  $app.save(order);
...
routerAdd("GET", "/api/nuanban/elder/stats", function (e) {
  ...
  return e.json(200, {
    ...
    caregiverNearbyCount: nb.countActiveCaregivers(),
  });
});
...
routerAdd("GET", "/api/nuanban/student/service-logs", function (e) {
  ...
  const list = [];
  for (let i = 0; i < records.length; i++) {
    list.push(nb.serviceLogDtoFromOrder(records[i]));
  }
  return e.json(200, { list: list });
});
...
routerAdd("GET", "/api/nuanban/student/settlements", function (e) {
  ...
  if (!rc.ok) return e.json(rc.code, rc.body);
  if (!e.auth) return e.json(401, { message: "需要登录" });
  return e.json(200, { list: nb.studentSettlementsForUser(e.auth.id) });
});
...
routerAdd("POST", "/api/nuanban/family/packages/purchase", function (e) {
  ...
  if (!auth) return e.json(401, { message: "需要登录" });
  if (nb.isFormalAuthMode()) {
    return e.json(400, { message: "暂无可用服务包" });
  }
  const raw = toString(e.request.body);
  const body = raw ? JSON.parse(raw) : {};
...
routerAdd("GET", "/api/nuanban/platform/activity", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  return e.json(200, { list: nb.platformActivityFromOrders(12) });
});
...
routerAdd("POST", "/api/nuanban/platform/seed-scenario", function (e) {
  ...
  var serviceName = "服务";
  try {
    elderName = nb.elderNameById(order.getString("elder"));
  } catch (_) {}
  try {
    serviceName = nb.serviceInfoById(order.getString("service_item")).name;
  } catch (_) {}
...
routerAdd("GET", "/api/nuanban/platform/organizations", function (e) {
  var list = [];
  try {
    var records = $app.findRecordsByFilter("organizations", "enabled = true", "name", 100, 0);
...
routerAdd("GET", "/api/nuanban/platform/elders", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  var q = e.request.url.query();
  var page = parseInt(q.get("page") || "1", 10);
  var pageSize = parseInt(q.get("pageSize") || "50", 10);
  ...
  var offset = (page - 1) * pageSize;
  var keyword = String(q.get("q") || "").trim();
  var incompleteOnly = String(q.get("incomplete") || "") === "1";
  var list = [];
  var total = 0;
  try {
    var allRecords = $app.findRecordsByFilter("elders", "enabled = true", "-id", 10000, 0);
    var matched = [];
    for (var mi = 0; mi < allRecords.length; mi++) {
      var row = nb.opsElderRowFromRecord(allRecords[mi], e);
      if (incompleteOnly && row.orgProfileComplete) continue;
      if (nb.opsElderMatchesKeyword(row, keyword)) matched.push(row);
    }
    total = matched.length;
    list = matched.slice(offset, offset + pageSize);
  } catch (err) {
...
routerAdd("GET", "/api/nuanban/platform/elders/{id}", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  var id = e.request.pathValue("id");
  try {
    var elder = $app.findRecordById("elders", id);
    return e.json(200, nb.opsElderRowFromRecord(elder, e));
  } catch (_) {
    return e.json(404, { message: "老人档案不存在" });
  }
});

routerAdd("PATCH", "/api/nuanban/platform/elders/{id}", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  var id = e.request.pathValue("id");
  var raw = toString(e.request.body);
  var body = raw ? JSON.parse(raw) : {};
  var elder;
  try {
    elder = $app.findRecordById("elders", id);
  } catch (_) {
    return e.json(404, { message: "老人档案不存在" });
  }
  try {
    nb.opsElderApplyPatch(elder, body);
  } catch (err) {
    return e.json(400, { message: String(err.message || err) });
  }
  $app.save(elder);
  return e.json(200, { ok: true, elder: nb.opsElderRowFromRecord(elder, e) });
});

routerAdd("POST", "/api/nuanban/platform/elders/{id}/avatar", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  var id = e.request.pathValue("id");
  var elder;
  try {
  ...
  var linked = nb.opsElderLinkedAuth(id);
  if (!linked.user) {
    return e.json(400, { message: "该档案未绑定登录账号，无法上传头像" });
  }
  var files = e.findUploadedFiles("avatar");
  if (!files || !files.length) return e.json(400, { message: "请选择图片" });
  linked.user.set("avatar", files[0]);
  $app.save(linked.user);
  var av = nb.userAvatarFields(linked.user, e);
  return e.json(200, { ok: true, avatarUrl: av.avatarUrl });
});
...
routerAdd("GET", "/api/nuanban/platform/service-catalog", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  return e.json(200, nb.listServiceCatalog());
});

routerAdd("POST", "/api/nuanban/platform/service-categories", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  var raw = toString(e.request.body);
  var body = raw ? JSON.parse(raw) : {};
  try {
    return e.json(200, { ok: true, category: nb.createServiceCategory(body) });
  } catch (err) {
    return e.json(400, { message: String(err.message || err) });
  }
});

routerAdd("POST", "/api/nuanban/platform/service-items", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  var raw = toString(e.request.body);
  var body = raw ? JSON.parse(raw) : {};
  try {
    return e.json(200, { ok: true, item: nb.upsertServiceItem(body, "") });
  } catch (err) {
    return e.json(400, { message: String(err.message || err) });
  }
});

routerAdd("PATCH", "/api/nuanban/platform/service-items/{id}", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  var id = e.request.pathValue("id");
  var raw = toString(e.request.body);
  var body = raw ? JSON.parse(raw) : {};
  try {
    return e.json(200, { ok: true, item: nb.upsertServiceItem(body, id) });
  } catch (err) {
    return e.json(400, { message: String(err.message || err) });
  }
});

routerAdd("GET", "/api/nuanban/platform/school-cooperation", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  var groups = nb.schoolCoopGroupedByOrg();
  return e.json(200, { groups: groups, total: groups.length });
});

routerAdd("POST", "/api/nuanban/platform/school-cooperation", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  var raw = toString(e.request.body);
  var body = raw ? JSON.parse(raw) : {};
  try {
    return e.json(200, { ok: true, item: nb.createSchoolCooperation(body) });
  } catch (err) {
    return e.json(400, { message: String(err.message || err) });
  }
});

routerAdd("POST", "/api/nuanban/platform/school-cooperation/{id}/disable", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  var id = e.request.pathValue("id");
  try {
    return e.json(200, { ok: true, item: nb.disableSchoolCooperation(id) });
  } catch (err) {
    return e.json(400, { message: String(err.message || err) });
  }
});

routerAdd("GET", "/api/nuanban/school-cooperation/partners", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  if (!e.auth) return e.json(401, { message: "需要登录" });
  return e.json(200, { byOrg: nb.schoolCoopPartnerMap() });
});
...
routerAdd("GET", "/api/nuanban/platform/overview", function (e) {
  ...
  var orderTotal = pending + pendingPay + inSvc + done + pendingConfirm;
  var matchRate = orderTotal > 0 ? Math.min(100, Math.round((done / orderTotal) * 100)) : (done > 0 ? 100 : 0);
  var caregiversCount = nb.countActiveCaregivers();
  return e.json(200, {
    ...
    eldersNearby: elders,
    todayMatches: inSvc + pendingConfirm,
    matchSuccessRatePct: matchRate,
    matchingPaths: [
      {
        id: "org_dispatch",
        label: "机构派单",
    ...
    coreCompletionPct: orderTotal > 0 ? matchRate : 0,
    auditStatus: "PASS",
...
routerAdd("GET", "/api/nuanban/orders/{id}/messages", function (e) {
  ...
  return e.json(200, {
    ...
    callOpen: nb.orderCallThreadOpen(order.getString("status")),
  });
});
...
routerAdd("GET", "/api/nuanban/orders/{id}/call", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  if (!e.auth) return e.json(401, { message: "需要登录" });
  var roleHdr = e.request.header.get("X-Active-Role") || "";
  var orderId = e.request.pathValue("id");
  ...
  if (!nb.orderChatCanAccess(e.auth.id, roleHdr, order)) {
    return e.json(403, { message: "无权发起通话" });
  }
  return e.json(200, nb.orderCallInfoDto(order, roleHdr));
});

routerAdd("POST", "/api/nuanban/orders/{id}/call", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  if (!e.auth) return e.json(401, { message: "需要登录" });
  var roleHdr = e.request.header.get("X-Active-Role") || "";
  var orderId = e.request.pathValue("id");
  ...
  if (!nb.orderCallThreadOpen(order.getString("status"))) {
    return e.json(400, { message: "待服务或服务进行中可语音通话" });
  }
  var info = nb.orderCallInfoDto(order, roleHdr);
  var log = nb.orderCallLog(orderId, e.auth.id, roleHdr);
  info.callId = log.id;
  info.loggedAt = log.at;
  info.clientId = nb.orderCallClientId(e.auth.id, orderId);
  return e.json(200, info);
});

routerAdd("GET", "/api/nuanban/orders/{id}/call/signal", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  if (!e.auth) return e.json(401, { message: "需要登录" });
  var roleHdr = e.request.header.get("X-Active-Role") || "";
  var orderId = e.request.pathValue("id");
  ...
  if (!nb.orderChatCanAccess(e.auth.id, roleHdr, order)) {
    return e.json(403, { message: "无权访问通话信令" });
  }
  var clientId = e.request.url.query().get("clientId") || "";
  if (!clientId) return e.json(400, { message: "缺少 clientId" });
  var since = e.request.url.query().get("since") || "0";
  return e.json(200, nb.orderCallSignalPoll(orderId, e.auth.id, clientId, since));
});

routerAdd("POST", "/api/nuanban/orders/{id}/call/signal", function (e) {
  var nb = require(__hooks + "/nuanban_lib.js");
  if (!e.auth) return e.json(401, { message: "需要登录" });
  var roleHdr = e.request.header.get("X-Active-Role") || "";
  var orderId = e.request.pathValue("id");
  ...
  if (!nb.orderChatCanAccess(e.auth.id, roleHdr, order)) {
    return e.json(403, { message: "无权发送通话信令" });
  }
  if (!nb.orderCallThreadOpen(order.getString("status"))) {
    return e.json(400, { message: "待服务或服务进行中可语音通话" });
  }
  ...
  if (!clientId) return e.json(400, { message: "缺少 clientId" });
  try {
    var sig = nb.orderCallSignalPost(orderId, e.auth.id, clientId, body);
    return e.json(200, { ok: true, signal: sig });
  } catch (err) {
    return e.json(400, { message: String(err.message || err) });
  }
});
