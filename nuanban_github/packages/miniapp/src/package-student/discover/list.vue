  ...
  <view class="page">
    ...
    <template v-else-if="mode === 'list'">
      ...
      <view v-else-if="!displayList.length" class="empty">
        ...
        <text v-if="!searchKeyword && !formalAuth" class="empty-hint">开发环境可先执行 ./scripts/seed-demo.sh</text>
        <text v-else-if="!searchKeyword && formalAuth" class="empty-hint">暂无老人档案，请通过运营台或老人注册添加</text>
      </view>
...
async function reload() {
  ...
  try {
    ...
    list.value = schoolCoopOnly.value
      ? filterEldersBySchoolCoop(mapped, schoolName.value, partners)
      : mapped;
  } catch (e) {
    list.value = [];
