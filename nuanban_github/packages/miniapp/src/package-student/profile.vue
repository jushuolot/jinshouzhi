<template>
  <view class="page">
    ...
    <view v-if="auditLocked" class="audit-banner">
      <text>审核已通过 · 资料已锁定，修改请前往「编辑资料」申请重新提交审核</text>
    </view>
...
const displayName = computed(() => studentGreetingName(profile.value));
  ...
  return [
    {
      title: '基本资料',
      rows: [
        { label: '显示名称', value: p.displayName || p.nickname || '-' },
        { label: '学校', value: p.schoolName || '-' },
        { label: '性别', value: p.gender || '-' },
        { label: '专业', value: p.major || '-' },
        { label: '年级', value: p.grade || '-' },
        { label: '联系手机号', value: p.contactPhone || '-' },
        { label: '学号', value: p.studentId || '-' },
        { label: '服务区域', value: serviceAreaSummary({ polygons: p.serviceAreaPolygons || [] }) },
        { label: '可服务时间', value: (p.serviceHours || []).join('、') || '-' },
      ],
    },
  ];
});
