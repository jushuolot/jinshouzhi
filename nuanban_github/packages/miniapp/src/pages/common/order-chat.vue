<template>
  <view class="page">
    <view class="banner">
      ...
      <text class="banner-sub">📞 待服务起可实时通话 · 按住说话为语音留言 · 服务结束后关闭</text>
      <OrderVoiceCallButton
        v-if="callOpen"
        class="banner-call"
        :order-id="orderId"
        :call-open="callOpen"
    ...
    <view v-if="threadOpen" class="composer">
      <view class="composer-hint">
        <text v-if="callOpen" class="hint-call">左侧「通话」= 实时语音；「按住说话」= 发语音条</text>
        <text v-else class="hint-muted">实时通话将在待服务/服务进行中开放</text>
      </view>
      <view class="composer-row">
        ...
        <input v-model="draft" class="input" placeholder="输入文字…" confirm-type="send" @confirm="send" />
...
import OrderVoiceCallButton from '../../components/OrderVoiceCallButton.vue';
...
async function reload() {
  ...
  try {
    ...
    callOpen.value = res.callOpen === true;
