    ...
    <view v-for="s in list" :key="s.userId" class="card">
      <view class="card-head">
        ...
        <view class="info">
          ...
          <text v-if="s.phone" class="meta">手机 {{ s.phone }}</text>
          <text v-if="s.contactPhone" class="meta">联系手机 {{ s.contactPhone }}</text>
          <text v-if="s.studentId" class="meta">学号 {{ s.studentId }}</text>
          <text v-if="s.serviceHours?.length" class="meta">可服务 {{ s.serviceHours.join('、') }}</text>
          <text class="meta">{{ s.major || '未填专业' }} · {{ s.grade || '未填年级' }}</text>
        </view>
...
function avatarUrl(s: OpsStudentProfile) {
  ...
  return resolveCartoonAvatarUrl(s.cartoonAvatarId, s.customCartoonAvatarUrl || s.displayName || s.nickname);
}
