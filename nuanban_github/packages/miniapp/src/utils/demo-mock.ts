...
import { isElderOrgProfileComplete, ELDER_ORG_PROFILE_HINT } from './elder-org-profile';
...
/** 运营台老人机构档案覆盖（Mock） */
const mockOpsElderOverrides: Record<
  string,
  {
    orgId?: string;
    orgName?: string;
...
function mockElderLoginPhone(id: string) {
  const richId = normalizeElderId(id, ELDERS);
  if (richId === 'elder-1') return '13500000005';
  return '';
}

function buildMockOpsElderRow(id: string) {
  const richId = normalizeElderId(id, ELDERS);
  const p = getRichElderProfile(richId);
  const base = ELDERS.find((e) => e.id === richId);
  const ov = mockOpsElderOverrides[id] || mockOpsElderOverrides[richId] || {};
  const orgId = ov.orgId || base?.org || DEMO_ORG_MAIN.id;
  const orgName = ov.orgName || (orgId ? orgNameById(orgId) : '');
  const district = ov.district ?? p?.district ?? '';
  const livingSituation = ov.livingSituation ?? p?.livingSituation ?? '';
  const healthStatus = ov.healthStatus ?? p?.healthStatus ?? '';
  const mobility = ov.mobility ?? p?.mobility ?? '';
  const isNew = id.startsWith('elder-new');
  const loginPhone = mockElderLoginPhone(id);
  const name = ov.name ?? (isNew ? elderProfileState.name || '老人' : p?.name || base?.name || '老人');
  return {
    id,
    userId: loginPhone ? `mock-user-${loginPhone}` : '',
    name,
    phone: ov.phone ?? loginPhone,
    loginPhone,
    avatarUrl: ov.avatarUrl ?? p?.avatarUrl ?? '',
    age: ov.age ?? p?.age ?? 0,
    gender: ov.gender ?? p?.gender ?? '',
    address: ov.address ?? p?.address ?? '',
    orgId: isNew && !ov.orgId ? '' : orgId,
    orgName: isNew && !ov.orgId ? '' : orgName,
    district: isNew && ov.district == null ? '' : district,
    livingSituation: isNew && ov.livingSituation == null ? '' : livingSituation,
    healthStatus: isNew && ov.healthStatus == null ? '' : healthStatus,
    mobility: isNew && ov.mobility == null ? '' : mobility,
    emergencyContactName: ov.emergencyContactName ?? p?.emergencyContact?.name ?? '',
    emergencyContactRelation: ov.emergencyContactRelation ?? p?.emergencyContact?.relation ?? '',
    emergencyContactPhone: ov.emergencyContactPhone ?? p?.emergencyContact?.phone ?? '',
    notes: ov.notes ?? '',
    latitude: ov.latitude ?? base?.latitude ?? p?.latitude ?? 0,
    longitude: ov.longitude ?? base?.longitude ?? p?.longitude ?? 0,
    enabled: true,
    orgProfileComplete: isElderOrgProfileComplete({
      orgName: isNew && !ov.orgId ? '' : orgName,
      district: isNew && ov.district == null ? '' : district,
      healthStatus: isNew && ov.healthStatus == null ? '' : healthStatus,
      mobility: isNew && ov.mobility == null ? '' : mobility,
...
function listMockOpsElders() {
  const list = ELDERS.map((e) => buildMockOpsElderRow(e.id));
  if (!elderProfileState.seeded && elderProfileState.name) {
    list.unshift(buildMockOpsElderRow('elder-new-local'));
  }
  return list;
}
...
const mockServiceCategoryAdds: Array<{ id: string; name: string; sortOrder: number }> = [];
const mockServiceItemAdds: Array<{
  id: string;
  categoryId: string;
  name: string;
  priceCents: number;
...
const mockServiceItemPatches: Record<
  string,
  Partial<{
    categoryId: string;
    name: string;
...
function buildMockServiceCatalog() {
  const categories = [
    ...
    ...mockServiceCategoryAdds.map((c) => ({ ...c, items: [] as typeof categories[0]['items'] })),
  ];
  const catMap: Record<string, (typeof categories)[number]> = {};
  for (const c of categories) catMap[c.id] = c;

  const baseItems = SERVICE_ITEMS.map((s) => {
    const patch = mockServiceItemPatches[s.id] || {};
    const categoryId = patch.categoryId ?? s.category;
    const priceCents = patch.priceCents ?? s.price_cents;
    return {
      id: s.id,
      ...
      durationMinutes: patch.durationMinutes ?? s.duration_minutes,
      requiresOutdoorApproval: patch.requiresOutdoorApproval ?? s.requires_outdoor_approval,
      enabled: patch.enabled ?? true,
    };
  });
  const allItems = [...baseItems, ...mockServiceItemAdds];
  const uncategorized: typeof baseItems = [];
  let enabledCount = 0;
  ...
  return { categories, uncategorized, totalItems: allItems.length, enabledCount };
}
  ...
  serviceAreaPolygons: [
    {
      ...
      ring: [
        ...
        { lat: 31.20, lng: 121.58 },
      ],
    },
  ] as { id: string; label?: string; ring: { lat: number; lng: number }[] }[],
  serviceHours: ['周一至周五 14:00–18:00', '周六 09:00–12:00'],
};
...
function resetRoleProfileState(role: RoleKey, displayName?: string) {
  if (role === 'student') {
    ...
    studentProfileState.studentId = '';
    studentProfileState.serviceAreaPolygons = [];
    studentProfileState.serviceHours = [];
  }
  if (role === 'family') {
...
function studentProfileDto() {
  ...
  const auditStatus = studentRoleStatus();
  const profile = {
    nickname: dto.displayName || dto.nickname,
    email: dto.email,
    ...
    customCartoonAvatarUrl: mockCustomCartoonByUser[userId] || undefined,
    avatarUrl: resolveCartoonAvatarUrl(cartoonId, mockCustomCartoonByUser[userId]),
    verificationPhotoUrl: verificationPhotoUrl || undefined,
    contactPhone: mockContactPhoneByUser[userId] || '',
    studentId: studentProfileState.studentId || undefined,
    serviceAreaPolygons: studentProfileState.serviceAreaPolygons,
    serviceHours: studentProfileState.serviceHours,
    profileComplete: isStudentAuditProfileComplete({
      displayName: dto.displayName,
      schoolName: dto.schoolName,
      contactPhone: mockContactPhoneByUser[userId] || '',
      verificationPhotoUrl: verificationPhotoUrl || undefined,
      cartoonAvatarId: cartoonId,
      customCartoonAvatarUrl: mockCustomCartoonByUser[userId],
      serviceAreaPolygons: studentProfileState.serviceAreaPolygons,
      serviceHours: studentProfileState.serviceHours,
    }),
    auditStatus,
...
function elderSelfProfileDto() {
  ...
  const dto = {
    id: base?.id || 'elder-new',
    name: elderProfileState.seeded ? base!.name : elderProfileState.name || '老人',
    age: elderProfileState.seeded ? base!.age : elderProfileState.age,
  ...
  const orgComplete = isElderOrgProfileComplete(dto);
  const withOrg = {
    ...dto,
    orgProfileComplete: orgComplete,
    orgProfileHint: orgComplete ? '' : ELDER_ORG_PROFILE_HINT,
  };
  const url = getMockAvatarUrl(USERS.elder.id);
  return url ? { ...withOrg, avatarUrl: url } : withOrg;
}
...
function settlementsForStudentUser(user: { email: string }): SettlementRecord[] {
  if (PRESET_DEMO_STUDENT_EMAILS.has(user.email.toLowerCase())) {
...
const mockContactPhoneByUser: Record<string, string> = {};
...
function mockCallOpen(status: string) {
  return status === 'pending_service' || status === 'in_service';
}

function mockOrderCallMaskedNumber(orderId: string) {
  let h = 0;
  for (let i = 0; i < orderId.length; i++) {
    h = ((h << 5) - h + orderId.charCodeAt(i)) | 0;
  }
  h = Math.abs(h);
  const ext = String(h % 10000000).padStart(7, '0');
  return `400${ext}`;
}

function mockOrderCallMaskedDisplay(num: string) {
  if (num.length < 10) return num;
  return `${num.slice(0, 3)}-${num.slice(3, 6)}-${num.slice(6)}`;
}

function mockOrderCallPeerAlias(order: MockOrder, callerRole?: RoleKey) {
  if (callerRole === 'student') {
    return order.family_user ? '家属·联系人' : '老人·本人';
  }
  return caregiverNameByUserId(order.student_user) || '陪护同学';
}

function mockOrderCallCanAccess(order: MockOrder) {
  const user = currentDemoUser();
  const role = currentActiveRole();
  if (role === 'student') return order.student_user === user.id;
  if (role === 'family') return order.family_user === user.id;
  if (role === 'elder') return order.elder === currentElderId();
  return false;
}

function mockOrderCallInfo(order: MockOrder) {
  const role = currentActiveRole();
  const open = mockCallOpen(order.status);
  const masked = mockOrderCallMaskedNumber(order.id);
  const user = currentDemoUser();
  return {
    callOpen: open,
    mode: 'webrtc' as const,
    roomId: `room_order_${order.id}`,
    iceServers: [{ urls: 'stun:stun.l.google.com:19302' }],
    clientId: `cid_${user.id.slice(-6)}_${order.id.slice(-4)}`,
    maskedNumber: masked,
    maskedNumberDisplay: mockOrderCallMaskedDisplay(masked),
    peerAlias: open ? mockOrderCallPeerAlias(order, role) : '',
    hint: open
...
const mockOrderCallSignals: Record<
  string,
  {
    roomId: string;
    status: string;
...
function mockOrderCallSignalSession(orderId: string) {
  if (!mockOrderCallSignals[orderId]) {
    mockOrderCallSignals[orderId] = {
      roomId: `room_order_${orderId}`,
      status: 'waiting',
      nextSeq: 1,
      signals: [],
    };
  }
  return mockOrderCallSignals[orderId];
}

function mockOrderCallSignalPoll(orderId: string, userId: string, clientId: string, since: number) {
  const sess = mockOrderCallSignalSession(orderId);
  const out = sess.signals.filter(
    (s) => s.seq > since && !(s.fromUser === userId && s.clientId === clientId),
  );
  const maxSeq = sess.signals.reduce((m, s) => Math.max(m, s.seq), since);
  return { signals: out, since: maxSeq, roomId: sess.roomId, status: sess.status };
}

function mockOrderCallSignalPost(
  orderId: string,
  userId: string,
  clientId: string,
  body: { type: string; sdp?: RTCSessionDescriptionInit; candidate?: RTCIceCandidateInit },
) {
  const sess = mockOrderCallSignalSession(orderId);
  ...
  if (body.type === 'hangup') sess.status = 'ended';
  else if (body.type === 'join' && sess.status === 'waiting') sess.status = 'active';
  return sig;
}

const mockOrderCalls: Record<
  string,
  Array<{ id: string; callerUser: string; callerRole: string; maskedNumber: string; at: string }>
> = {};
...
function pendingOrderDto(o: MockOrder) {
  ...
  return {
    ...
    callOpen: mockCallOpen(o.status),
  };
}
...
export async function demoMockRequest<T>(options: UniApp.RequestOptions): Promise<T> {
  ...
  if (method === 'POST' && path === '/nuanban/auth/register') {
    ...
    if (role === 'student') {
      ...
      if (data.schoolName) {
        const sn = String(data.schoolName).trim();
        if (isKnownSchool(sn)) studentProfileState.schoolName = sn;
      }
      if (data.cartoonAvatarId) {
        try {
          const stored = uni.getStorageSync(DEMO_STORAGE_USER) as PhoneSession['user'];
          if (stored?.id) setMockCartoonAvatarId(stored.id, String(data.cartoonAvatarId));
        } catch {
          /* ignore */
        }
      }
      if (Array.isArray(data.serviceAreaPolygons)) {
        studentProfileState.serviceAreaPolygons = data.serviceAreaPolygons;
      }
      if (Array.isArray(data.serviceHours)) {
        studentProfileState.serviceHours = data.serviceHours.map(String);
      }
      if (data.studentId) studentProfileState.studentId = String(data.studentId);
    }
    ...
    if (data.contactPhone) mockContactPhoneByUser[user.id] = String(data.contactPhone);
    else if (data.wechatId) mockContactPhoneByUser[user.id] = String(data.wechatId);
  ...
  if (method === 'GET' && path === '/nuanban/student/settlements') {
    ...
    if (roleErr) return Promise.reject({ message: roleErr, statusCode: 403 });
    const user = demoUserForRoles(mockRegisterRoles);
    return delay({ list: [...settlementsForStudentUser(user)].reverse() } as T);
  }
  if (method === 'GET' && path === '/nuanban/student/withdrawal') {
    const roleErr = assertDemoActiveRole(options, path, 'student');
    if (roleErr) return Promise.reject({ message: roleErr, statusCode: 403 });
    const user = demoUserForRoles(mockRegisterRoles);
    return delay(getStudentWithdrawalOverview(user.id, settlementsForStudentUser(user)) as T);
  }
  if (method === 'POST' && path === '/nuanban/student/withdrawal') {
    ...
    try {
      const overview = submitStudentWithdrawal(
        user.id,
        settlementsForStudentUser(user),
  ...
  if (method === 'GET' && path === '/nuanban/schools') {
    return delay({ list: [...KNOWN_SCHOOLS] } as T);
  }
  ...
  if (method === 'PATCH' && path === '/nuanban/student/profile') {
    ...
    if (data.contactPhone != null) {
      mockContactPhoneByUser[currentDemoUser().id] = String(data.contactPhone);
    } else if (data.wechatId != null) {
      mockContactPhoneByUser[currentDemoUser().id] = String(data.wechatId);
    }
    if (data.studentId != null) studentProfileState.studentId = String(data.studentId);
    if (Array.isArray(data.serviceAreaPolygons)) {
      studentProfileState.serviceAreaPolygons = data.serviceAreaPolygons;
    }
    if (Array.isArray(data.serviceHours)) {
    ...
    if (resubmitAudit && (status === 'active' || status === 'rejected')) {
      const roles = mockRegisterRoles.length ? [...mockRegisterRoles] : readStoredRoles();
      const st = roles.find((r) => r.role === 'student');
      if (st) {
        st.status = 'pending';
        persistDemoRoles(roles);
      }
    }
    return delay({ ok: true, ...studentProfileDto(), profileComplete: studentProfileComplete() } as T);
  }
  if (method === 'POST' && path === '/nuanban/student/cartoon-avatar') {
    const roleErr = assertDemoActiveRole(options, path, 'student');
    if (roleErr) return Promise.reject({ message: roleErr, statusCode: 403 });
    ...
    return delay({ ok: true, customCartoonAvatarUrl: url } as T);
  }
  ...
  if (method === 'GET' && path === '/nuanban/elder/caregivers/nearby') {
    const lat = parseFloat(query.get('lat') || '31.23');
    const lng = parseFloat(query.get('lng') || '121.47');
    const radiusKmRaw = query.get('radiusKm');
    const radiusKm = radiusKmRaw != null && radiusKmRaw !== '' ? parseFloat(radiusKmRaw) : null;
    const gender = query.get('gender') || '';
    const school = (query.get('school') || '').trim().toLowerCase();
    const kw = (query.get('q') || '').trim().toLowerCase();

    const all = CAREGIVERS.map((c) => {
      const item = caregiverToListItem(c);
      if (!lat || !lng || c.distanceKm == null) return item;
      return item;
    });
    ...
    const filtered = all.filter((item) => {
      ...
      if (kw) {
        const hay = [item.name, item.school, item.gender, item.distance, ...(item.tags || [])]
          .join(' ')
          .toLowerCase();
    ...
    return delay({ list: filtered, total: all.length, filtered: filtered.length } as T);
  }
  ...
  if (method === 'POST' && path === '/nuanban/elder/orders') {
    ...
    let studentUser = String(data.studentId || data.studentUserId || '');
    if (studentUser) {
      const cg = CAREGIVERS.find((c) => c.id === studentUser || c.userId === studentUser);
      if (cg) studentUser = cg.userId;
    }
    state.orders.push({
      id,
      elder: normalizeElderId(String(data.elderId || 'elder-zhang'), ELDERS),
      service_item: svc.id,
      status: needsOutdoor ? 'outdoor_pending' : 'pending_accept',
      amount_cents: svc.price_cents,
      payment_status: 'paid',
      family_user: familyUserId,
      student_user: studentUser || undefined,
    ...
    if (needsOutdoor) {
      ...
      mockAppendTimeline(id, 'pending_accept', '等待同学接单', 'system');
    }
    if (needsOutdoor) {
      state.outdoorApprovals.push({
        id: `outdoor-${Date.now()}`,
  ...
  if (method === 'GET' && path === '/nuanban/platform/organizations') {
    return delay({
      list: [
        { id: DEMO_ORG_MAIN.id, name: DEMO_ORG_MAIN.name },
        { id: 'org-east', name: '城东颐养中心' },
      ],
    } as T);
  }

  if (method === 'GET' && path === '/nuanban/platform/service-catalog') {
    return delay(buildMockServiceCatalog() as T);
  }

  if (method === 'POST' && path === '/nuanban/platform/service-categories') {
    const name = String(data.name || '').trim();
    if (!name) return Promise.reject({ message: '请输入类目名称', statusCode: 400 });
    const existing = [...SERVICE_CATEGORIES, ...mockServiceCategoryAdds].find((c) => c.name === name);
    if (existing) {
      const dto = 'sort_order' in existing
        ? { id: existing.id, name: existing.name, sortOrder: existing.sort_order }
        : existing;
      return delay({ ok: true, category: dto } as T);
    }
    const sortOrder = parseInt(String(data.sortOrder ?? ''), 10);
    const category = {
      id: `cat-mock-${Date.now()}`,
      name,
      sortOrder: Number.isFinite(sortOrder) ? sortOrder : mockServiceCategoryAdds.length + 10,
    };
    mockServiceCategoryAdds.push(category);
    return delay({ ok: true, category } as T);
  }

  if (method === 'POST' && path === '/nuanban/platform/service-items') {
    const categoryId = String(data.categoryId || '').trim();
    const name = String(data.name || '').trim();
    const priceCents = parseInt(String(data.priceCents ?? ''), 10);
    if (!categoryId) return Promise.reject({ message: '请选择服务类目', statusCode: 400 });
    if (!name) return Promise.reject({ message: '请输入服务名称', statusCode: 400 });
    if (!Number.isFinite(priceCents) || priceCents < 0) {
      return Promise.reject({ message: '请填写有效价格', statusCode: 400 });
    }
    const cat =
      SERVICE_CATEGORIES.find((c) => c.id === categoryId) ||
      mockServiceCategoryAdds.find((c) => c.id === categoryId);
    if (!cat) return Promise.reject({ message: '服务类目不存在', statusCode: 400 });
    ...
    const item = {
      ...
      requiresOutdoorApproval: !!data.requiresOutdoorApproval,
      enabled: data.enabled == null ? true : !!data.enabled,
    };
    mockServiceItemAdds.push(item);
    return delay({ ok: true, item } as T);
  }

  const serviceItemPatch = path.match(/^\/nuanban\/platform\/service-items\/([^/]+)$/);
  if (method === 'PATCH' && serviceItemPatch) {
    const id = serviceItemPatch[1];
    const base = SERVICE_ITEMS.find((s) => s.id === id);
    const added = mockServiceItemAdds.find((s) => s.id === id);
    if (!base && !added) return Promise.reject({ message: '服务项不存在', statusCode: 404 });
    const prev = mockServiceItemPatches[id] || {};
    const next = {
      ...prev,
      ...(data.categoryId != null ? { categoryId: String(data.categoryId) } : {}),
      ...(data.name != null ? { name: String(data.name) } : {}),
      ...(data.priceCents != null ? { priceCents: parseInt(String(data.priceCents), 10) } : {}),
      ...(data.durationMinutes != null
        ? { durationMinutes: parseInt(String(data.durationMinutes), 10) }
        : {}),
      ...(data.requiresOutdoorApproval != null
        ? { requiresOutdoorApproval: !!data.requiresOutdoorApproval }
        : {}),
      ...(data.enabled != null ? { enabled: !!data.enabled } : {}),
    };
    mockServiceItemPatches[id] = next;
    const catalog = buildMockServiceCatalog();
    const item =
      catalog.categories.flatMap((c) => c.items).find((i) => i.id === id) ||
      catalog.uncategorized.find((i) => i.id === id);
    if (!item) return Promise.reject({ message: '服务项不存在', statusCode: 404 });
    if (added) Object.assign(added, item);
    return delay({ ok: true, item } as T);
  }

  if (method === 'GET' && path === '/nuanban/platform/elders') {
    let filtered = listMockOpsElders();
    if (query.get('incomplete') === '1') {
      filtered = filtered.filter((e) => !e.orgProfileComplete);
    }
    const kw = (query.get('q') || '').trim().toLowerCase();
    if (kw) {
      filtered = filtered.filter((e) => {
        ...
        if (digits.length >= 4) {
          const phones = `${e.phone || ''} ${e.loginPhone || ''}`.replace(/\D/g, ' ');
  ...
  const opsElderGet = path.match(/^\/nuanban\/platform\/elders\/([^/]+)$/);
  if (method === 'GET' && opsElderGet) {
    const id = opsElderGet[1];
    const row = listMockOpsElders().find((e) => e.id === id) || buildMockOpsElderRow(normalizeElderId(id, ELDERS));
    return delay(row as T);
  }

  if (method === 'PATCH' && opsElderGet) {
    const id = opsElderGet[1];
    const prev = mockOpsElderOverrides[id] || {};
    const orgId = data.orgId != null ? String(data.orgId) : prev.orgId;
    const orgNameIn = data.orgName != null ? String(data.orgName) : prev.orgName;
    mockOpsElderOverrides[id] = {
      ...prev,
      orgId,
      orgName: orgNameIn || (orgId ? orgNameById(orgId) : prev.orgName),
      name: data.name != null ? String(data.name) : prev.name,
      phone: data.phone != null ? String(data.phone) : prev.phone,
      age: data.age != null ? Number(data.age) : prev.age,
      gender: data.gender != null ? String(data.gender) : prev.gender,
      address: data.address != null ? String(data.address) : prev.address,
      district: data.district != null ? String(data.district) : prev.district,
      livingSituation: data.livingSituation != null ? String(data.livingSituation) : prev.livingSituation,
      healthStatus: data.healthStatus != null ? String(data.healthStatus) : prev.healthStatus,
      mobility: data.mobility != null ? String(data.mobility) : prev.mobility,
      emergencyContactName:
        data.emergencyContactName != null ? String(data.emergencyContactName) : prev.emergencyContactName,
      emergencyContactRelation:
        data.emergencyContactRelation != null
          ? String(data.emergencyContactRelation)
          : prev.emergencyContactRelation,
      emergencyContactPhone:
        data.emergencyContactPhone != null ? String(data.emergencyContactPhone) : prev.emergencyContactPhone,
    ...
    const elder = buildMockOpsElderRow(id);
    return delay({ ok: true, elder } as T);
  }

  const opsElderAvatar = path.match(/^\/nuanban\/platform\/elders\/([^/]+)\/avatar$/);
  if (method === 'POST' && opsElderAvatar) {
    const id = opsElderAvatar[1];
    const prev = mockOpsElderOverrides[id] || {};
    mockOpsElderOverrides[id] = { ...prev, avatarUrl: 'data:image/png;base64,mock-ops-avatar' };
    return delay({ ok: true, avatarUrl: 'data:image/png;base64,mock-ops-avatar' } as T);
  }
  ...
  if (method === 'GET' && orderMsgGet) {
    ...
    return delay({ list, threadOpen: mockChatOpen(order.status), callOpen: mockCallOpen(order.status) } as T);
  }
  ...
  const orderCallSignalPath = path.match(/^\/nuanban\/orders\/([^/]+)\/call\/signal$/);
  if (orderCallSignalPath) {
    const oid = orderCallSignalPath[1];
    const order = state.orders.find((o) => o.id === oid);
    if (!order) return Promise.reject({ message: '订单不存在' });
    if (!mockOrderCallCanAccess(order)) {
      return Promise.reject({ message: '无权访问通话信令', statusCode: 403 });
    }
    const sender = currentChatSender();
    if (method === 'GET') {
      const clientId = String(query.get('clientId') || '');
      if (!clientId) return Promise.reject({ message: '缺少 clientId', statusCode: 400 });
      const since = parseInt(String(query.get('since') || '0'), 10) || 0;
      return delay(
        mockOrderCallSignalPoll(oid, sender.userId, clientId, since) as T,
      );
    }
    if (method === 'POST') {
      if (!mockCallOpen(order.status)) {
        return Promise.reject({ message: '待服务或服务进行中可语音通话', statusCode: 400 });
      }
      const body = (data || {}) as {
        type?: string;
        clientId?: string;
      ...
      if (!clientId) return Promise.reject({ message: '缺少 clientId', statusCode: 400 });
      const sig = mockOrderCallSignalPost(oid, sender.userId, clientId, {
        type: String(body.type || ''),
        sdp: body.sdp,
        candidate: body.candidate,
      });
      return delay({ ok: true, signal: sig } as T);
    }
  }

  const orderCallPath = path.match(/^\/nuanban\/orders\/([^/]+)\/call$/);
  if (orderCallPath) {
    const oid = orderCallPath[1];
    const order = state.orders.find((o) => o.id === oid);
    if (!order) return Promise.reject({ message: '订单不存在' });
    if (!mockOrderCallCanAccess(order)) {
      return Promise.reject({ message: '无权发起通话', statusCode: 403 });
    }
    const info = mockOrderCallInfo(order);
    if (method === 'GET') {
      return delay(info as T);
    }
    if (method === 'POST') {
      if (!info.callOpen) {
        return Promise.reject({ message: '待服务或服务进行中可语音通话', statusCode: 400 });
      ...
      const sender = currentChatSender();
      if (!mockOrderCalls[oid]) mockOrderCalls[oid] = [];
      const log = {
        id: `call_${oid}_${mockOrderCalls[oid].length + 1}`,
        callerUser: sender.userId,
        callerRole: sender.role,
        maskedNumber: info.maskedNumber,
        at: new Date().toISOString(),
      };
      mockOrderCalls[oid].push(log);
      return delay({ ...info, callId: log.id, loggedAt: log.at } as T);
    }
  }
