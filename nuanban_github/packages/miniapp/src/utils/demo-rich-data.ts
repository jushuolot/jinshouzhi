...
/** GitHub Pages Mock 专用演示号（正式环境不使用） */
...
export const DEMO_TEST_PHONES: DemoTestPhone[] = [
  {
    phone: '13500000001',
    email: 'm13500000001@test.nuanban.dev',
    role: 'student',
    label: '林黛玉 · 学生',
    testHint: '接单、签到、收入、服务中订单',
  },
  {
    phone: '13800000004',
  ...
  {
    phone: '13500000005',
    email: 'm13500000005@test.nuanban.dev',
    role: 'elder',
    label: '洪七公 · 老人',
...
/** 演示号学生（含多角色）保留结算/提现演示数据；新注册手机号应为 0 */
export const PRESET_DEMO_STUDENT_EMAILS = new Set(
  DEMO_TEST_PHONES.filter((p) => p.role === 'student' || p.role === 'multi').map((p) => p.email),
);
