<template>
  <view class="page" :class="{ 'elder-mode': role === 'elder', [elderFontCls]: role === 'elder' }">
    ...
    <view v-if="role === 'elder' && showOrgProfileHint" class="org-hint-banner">
      <text class="org-hint-text">{{ elderOrgHintText }}</text>
    </view>
...
import {
  ELDER_ORG_EMPTY_LABEL,
  ELDER_ORG_PROFILE_HINT,
  isElderOrgProfileComplete,
} from '../utils/elder-org-profile';
...
const showOrgProfileHint = computed(
  () => props.role === 'elder' && !isElderOrgProfileComplete(elderProfile.value),
);
const elderOrgHintText = computed(
  () => elderProfile.value?.orgProfileHint || ELDER_ORG_PROFILE_HINT,
);
...
const displayName = computed(() => {
  ...
  return roleStore.user?.nickname || '用户';
});
...
const subLine = computed(() => {
  ...
  if (props.role === 'elder') return elderProfile.value?.orgName || '未指定机构';
  return '';
});
...
const detailSections = computed((): ProfileDetailSection[] => {
  ...
  if (props.role === 'elder' && elderProfile.value) {
    ...
    const orgVal = (v?: string) => v?.trim() || ELDER_ORG_EMPTY_LABEL;
...
const menuItems = computed(() => {
  ...
  if (props.role === 'family') {
    return [
      ...
      { label: '服务记录', action: () => uni.navigateTo({ url: '/package-family/service/log' }) },
  ...
  if (props.role === 'elder') {
    return [
      ...
      { label: '服务记录', action: () => uni.navigateTo({ url: '/package-elder/service/log' }) },
