...
NUANBAN_FORMAL_URL=https://nuanban.cc/#/pages/common/login
...
- **Mac 上**：自动 `git push` 到 GitHub；若 `config/formal.env` 配了 `NUANBAN_SSH`，会 SSH 连服务器部署；**没配 SSH** 时脚本会打印 Workbench 里要粘贴的那一条命令  
