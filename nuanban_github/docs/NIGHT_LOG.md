...
## 本轮（2026-06-05 · 全角色冒烟 + 机构档案 + 语音 MVP 验收）

1. **自动化**：`test-product-smoke.mjs` **29/29**（老人/家属/运营/语音）；`test-student-audit-flow.mjs` **20/20**
2. **运营机构 Tab**：老人机构档案维护（`ops-org` / `ops-elder-profiles`），PATCH 区域/健康/居住/所属机构
3. **老人 Plan A**：注册自动建空 `elders` 档案；未填字段显示「待机构补充」；运营补齐后老人端同步
4. **模块地图 / 安全中心** 迁入运营 **更多**（需运营会话）
5. **语音通话 MVP**：`in_service` 订单详情 + 聊天页「语音通话（隐私号）」；`uni.makePhoneCall` + 聊天语音消息；须 `X-Active-Role`
6. **文档**：`LOCAL_TEST.md` 验收记录表、冒烟覆盖范围、运营/语音预期

验收：`./scripts/dev-test.sh` → 两脚本 0 失败 → H5 正式版逐点清单（见 LOCAL_TEST 推荐顺序）

## 本轮（Phase 20 · 储值卡预存与支付）

1. **储值卡** 页：`package-family|elder/wallet/index` · 余额 / 快捷充值 / 消费记录
2. 我的 → **储值卡** 入口带余额角标
3. 待支付与「确认服务并付款」支持 **储值卡支付**（余额充足时默认）
4. API：`GET|POST /nuanban/family|elder/wallet/*` · mock + pb hooks 对齐

验收：`13800000004` 我的 → 储值卡 → 充值 ¥200 → 待确认订单储值支付

## 本轮（Phase 21 · 学生端提现通道）

1. **可提现余额** = 已打款结算 − 已提现（演示 ¥533.00 起）
2. 新页 `package-student/withdrawal/index` · 微信即时 / 银行卡 1–3 日 mock
3. API：`GET|POST /nuanban/student/withdrawal` · mock + pb hooks 对齐
4. **我的 → 收入/提现** · 收入页横幅「去提现」

验收：`13800000001` → 我的 → 收入/提现 → 提现 ¥50 → 记录与余额变化

## 上一轮（Phase 19 · 家属/老人确认服务并付款）
