...
(function () {
  ...
  var CONFIG = {
    ...
    campDebateTranscript: { amount: 0.021, phases: ["超越期"], universeDayMod: 2 },
    bounty: {
      label: "任务赏金(演示)",
  ...
  var registry = {
    ...
    camp_debate_transcript: {
      id: "camp_debate_transcript",
      label: "营地拌嘴转写",
      source: "network_settlement",
      description: "周二拌嘴日整理营地语音口供，模拟资料转写入账",
    },
  };
  ...
  function loadState() {
    try {
      ...
      state.lastCampDebateTranscript = data.lastCampDebateTranscript || "";
    } catch (e) {
      // ignore
    }
  }
    ...
    try {
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          lastCampDebateTranscript: state.lastCampDebateTranscript,
          updatedAt: Date.now(),
        })
      );
    } catch (e) {
  ...
  function tryCampDebateTranscript() {
    var evo = typeof window !== "undefined" && window.MATCH3_EVOLUTION ? window.MATCH3_EVOLUTION : {};
    var uday = evo.universeDay || 0;
    var cfg = CONFIG.campDebateTranscript;
    if (!cfg || uday % 7 !== cfg.universeDayMod) return 0;
    var key = todayKey() + "-g" + (evo.generation || "x") + "-u" + uday;
    if (state.lastCampDebateTranscript === key) return 0;
    var clock =
      typeof window !== "undefined" && window.MATCH3_CIVILIZATION_CLOCK
        ? window.MATCH3_CIVILIZATION_CLOCK
        : null;
    var phase = clock ? clock.getPhase() : evo.civilizationPhase || "超越期";
    if (cfg.phases.indexOf(phase) < 0) return 0;
    state.lastCampDebateTranscript = key;
    saveState();
    var civ = clock ? clock.getCivilizationDate() : { civilizationYear: evo.civilizationYear || 2049 };
    return creditChannel("camp_debate_transcript", {
      amount: cfg.amount,
      meta: {
        kind: "camp_debate_transcript",
        universeDay: uday,
        generation: evo.generation || 0,
        phase: phase,
        year: civ.civilizationYear,
