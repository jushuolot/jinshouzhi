...
(function () {
  ...
  var CONFIG = {
    ...
    campKitchenRelay: { amount: 0.021, phases: ["超越期"], universeDayMod: 4 },
    bounty: {
      label: "任务赏金(演示)",
  ...
  var registry = {
    ...
    camp_kitchen_relay: {
      id: "camp_kitchen_relay",
      label: "营地伙食口述档案",
      source: "network_settlement",
      description: "周四生活日整理伙食、天气与公开文物图鉴引用，模拟档案入账",
    },
  };
  ...
  function loadState() {
    try {
      ...
      state.lastCampKitchenRelay = data.lastCampKitchenRelay || "";
    } catch (e) {
      // ignore
    }
  }
    ...
    try {
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          lastCampKitchenRelay: state.lastCampKitchenRelay,
          updatedAt: Date.now(),
        })
      );
    } catch (e) {
  ...
  function tryCampKitchenRelay() {
    var evo = typeof window !== "undefined" && window.MATCH3_EVOLUTION ? window.MATCH3_EVOLUTION : {};
    var uday = evo.universeDay || 0;
    var cfg = CONFIG.campKitchenRelay;
    if (!cfg || uday % 7 !== cfg.universeDayMod) return 0;
    var key = todayKey() + "-g" + (evo.generation || "x") + "-u" + uday;
    if (state.lastCampKitchenRelay === key) return 0;
    var clock =
      typeof window !== "undefined" && window.MATCH3_CIVILIZATION_CLOCK
        ? window.MATCH3_CIVILIZATION_CLOCK
        : null;
    var phase = clock ? clock.getPhase() : evo.civilizationPhase || "超越期";
    if (cfg.phases.indexOf(phase) < 0) return 0;
    state.lastCampKitchenRelay = key;
    saveState();
    var civ = clock ? clock.getCivilizationDate() : { civilizationYear: evo.civilizationYear || 2053 };
    return creditChannel("camp_kitchen_relay", {
      amount: cfg.amount,
      meta: {
        kind: "camp_kitchen_oral_archive",
        universeDay: uday,
        generation: evo.generation || 0,
        phase: phase,
        year: civ.civilizationYear,
