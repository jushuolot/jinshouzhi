...
(function () {
  ...
  var registry = {
    ...
    camp_kitchen_archive: {
      ...
      label: "营地炊烟档案",
      source: "network_settlement",
      description: "周四馋嘴/生活日归档灶火湿度与队员口述，模拟生活档案入账",
    },
  };
  ...
  function tryCampKitchenArchive() {
    ...
    saveState();
    var civ = clock ? clock.getCivilizationDate() : { civilizationYear: evo.civilizationYear || 2053 };
    var credited = creditChannel("camp_kitchen_archive", {
      amount: cfg.amount,
      meta: {
        kind: "camp_kitchen_archive",
        universeDay: uday,
        generation: evo.generation || 0,
        phase: phase,
        year: civ.civilizationYear,
    ...
    if (credited > 0 && typeof window.showSystemToast === "function") {
      window.showSystemToast("🍲 营地炊烟档案 · 文明历 " + civ.civilizationYear + " · 入账", 3600);
    }
    return credited;
  }
