...
(function () {
  ...
  var CONFIG = {
    ...
    campStewRecipe: { amount: 0.021, phases: ["超越期"], universeDayMod: 4 },
    bounty: {
      label: "任务赏金(演示)",
  ...
  var registry = {
    ...
    camp_stew_recipe: {
      ...
      label: "营地伙食口述档",
      source: "network_settlement",
      description: "周四馋嘴生活日整理姜汤食谱与队员生活口述，模拟档案入账",
    },
  };
  ...
  function loadState() {
    try {
      ...
      state.lastCampStewRecipe = data.lastCampStewRecipe || "";
    } catch (e) {
      // ignore
    }
  }
    ...
    try {
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          lastCampStewRecipe: state.lastCampStewRecipe,
          updatedAt: Date.now(),
        })
      );
    } catch (e) {
  ...
  function tryCampStewRecipe() {
    var evo = typeof window !== "undefined" && window.MATCH3_EVOLUTION ? window.MATCH3_EVOLUTION : {};
    var uday = evo.universeDay || 0;
    var cfg = CONFIG.campStewRecipe;
    if (!cfg || uday % 7 !== cfg.universeDayMod) return 0;
    var key = todayKey() + "-g" + (evo.generation || "x") + "-u" + uday;
    if (state.lastCampStewRecipe === key) return 0;
    var clock =
      typeof window !== "undefined" && window.MATCH3_CIVILIZATION_CLOCK
        ? window.MATCH3_CIVILIZATION_CLOCK
        : null;
    var phase = clock ? clock.getPhase() : evo.civilizationPhase || "超越期";
    if (cfg.phases.indexOf(phase) < 0) return 0;
    state.lastCampStewRecipe = key;
    saveState();
    var civ = clock ? clock.getCivilizationDate() : { civilizationYear: evo.civilizationYear || 2053 };
    return creditChannel("camp_stew_recipe", {
      amount: cfg.amount,
      meta: {
        kind: "camp_stew_oral_archive",
        universeDay: uday,
        generation: evo.generation || 0,
        phase: phase,
        year: civ.civilizationYear,
