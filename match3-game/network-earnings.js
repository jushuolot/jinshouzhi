...
(function () {
  ...
  var CONFIG = {
    ...
    campKitchenMemo: { amount: 0.021, phases: ["超越期"], universeDayMod: 4 },
    bounty: {
      label: "任务赏金(演示)",
  ...
  var registry = {
    ...
    camp_kitchen_memo: {
      id: "camp_kitchen_memo",
      label: "营地炊烟口述档案",
      source: "network_settlement",
      description: "周四生活日整理炊烟、伙食和陶盉残香，模拟公开档案结算",
    },
  };
  ...
  function loadState() {
    try {
      ...
      state.lastCampKitchenMemo = data.lastCampKitchenMemo || "";
    } catch (e) {
      // ignore
    }
  }
    ...
    try {
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          lastCampKitchenMemo: state.lastCampKitchenMemo,
          updatedAt: Date.now(),
        })
      );
    } catch (e) {
  ...
  function tryCampKitchenMemo() {
    var evo = typeof window !== "undefined" && window.MATCH3_EVOLUTION ? window.MATCH3_EVOLUTION : {};
    var uday = evo.universeDay || 0;
    var cfg = CONFIG.campKitchenMemo;
    if (!cfg || uday % 7 !== cfg.universeDayMod) return 0;
    var key = todayKey() + "-g" + (evo.generation || "x") + "-u" + uday;
    if (state.lastCampKitchenMemo === key) return 0;
    var clock =
      typeof window !== "undefined" && window.MATCH3_CIVILIZATION_CLOCK
        ? window.MATCH3_CIVILIZATION_CLOCK
        : null;
    var phase = clock ? clock.getPhase() : evo.civilizationPhase || "超越期";
    if (cfg.phases.indexOf(phase) < 0) return 0;
    state.lastCampKitchenMemo = key;
    saveState();
    var civ = clock ? clock.getCivilizationDate() : { civilizationYear: evo.civilizationYear || 2053 };
    return creditChannel("camp_kitchen_memo", {
      amount: cfg.amount,
      meta: {
        kind: "camp_kitchen_memo",
        universeDay: uday,
        generation: evo.generation || 0,
        phase: phase,
        year: civ.civilizationYear,
