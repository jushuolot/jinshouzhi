...
(function () {
  ...
  var CONFIG = {
    ...
    newPitSignal: { amount: 0.024, phases: ["超越期"], universeDayMod: 1 },
    bounty: {
      label: "任务赏金(演示)",
  ...
  var registry = {
    ...
    new_pit_signal: {
      ...
      label: "新坑声纹结算",
      source: "network_settlement",
      description: "周一下新坑时，同步超越期声纹档案的外部结算",
    },
  };
  ...
  function tryNewPitSignal() {
    ...
    if (cfg.phases.indexOf(phase) < 0) return 0;
    var civ = clock ? clock.getCivilizationDate() : { civilizationYear: 0 };
    ...
    if (credited > 0 && typeof window.showSystemToast === "function") {
      window.showSystemToast("📡 新坑声纹同步 · 文明历 " + civ.civilizationYear + " · 入账", 3600);
    }
    return credited;
  }
