...
(function () {
  ...
  var CONFIG = {
    ...
    campSupperDispatch: { amount: 0.021, phases: ["超越期"], universeDayMod: 4 },
    bounty: {
      label: "任务赏金(演示)",
  ...
  var registry = {
    ...
    camp_supper_dispatch: {
      id: "camp_supper_dispatch",
      label: "营地补给清单",
      source: "network_settlement",
      description: "周四馋嘴生活日整理营地食谱与陶盉谷物样本，模拟补给档案入账",
    },
  };
  ...
  function loadState() {
    try {
      ...
      state.lastCampSupperDispatch = data.lastCampSupperDispatch || "";
    } catch (e) {
      // ignore
    }
  }
    ...
    try {
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          lastCampSupperDispatch: state.lastCampSupperDispatch,
          updatedAt: Date.now(),
        })
      );
    } catch (e) {
  ...
  function tryCampSupperDispatch() {
    var evo = typeof window !== "undefined" && window.MATCH3_EVOLUTION ? window.MATCH3_EVOLUTION : {};
    var uday = evo.universeDay || 0;
    var cfg = CONFIG.campSupperDispatch;
    if (!cfg || uday % 7 !== cfg.universeDayMod) return 0;
    var key = todayKey() + "-g" + (evo.generation || "x") + "-u" + uday;
    if (state.lastCampSupperDispatch === key) return 0;
    var clock =
      typeof window !== "undefined" && window.MATCH3_CIVILIZATION_CLOCK
        ? window.MATCH3_CIVILIZATION_CLOCK
        : null;
    var phase = clock ? clock.getPhase() : evo.civilizationPhase || "超越期";
    if (cfg.phases.indexOf(phase) < 0) return 0;
    state.lastCampSupperDispatch = key;
    saveState();
    var civ = clock ? clock.getCivilizationDate() : { civilizationYear: evo.civilizationYear || 2053 };
    return creditChannel("camp_supper_dispatch", {
      amount: cfg.amount,
      meta: {
        kind: "camp_supper_dispatch",
        universeDay: uday,
        generation: evo.generation || 0,
        phase: phase,
        year: civ.civilizationYear,
