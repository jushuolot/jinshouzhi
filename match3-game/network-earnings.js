...
(function () {
  ...
  var CONFIG = {
    ...
    campSupperSupply: { amount: 0.021, phases: ["超越期"], universeDayMod: 4 },
    bounty: {
      label: "任务赏金(演示)",
  ...
  var registry = {
    ...
    camp_supper_supply: {
      id: "camp_supper_supply",
      label: "营地补给联营",
      source: "network_settlement",
      description: "周四馋嘴日整理营地补给清单与陶盉生活档案，模拟联营结算",
    },
  };
  ...
  function loadState() {
    try {
      ...
      state.lastCampSupperSupply = data.lastCampSupperSupply || "";
    } catch (e) {
      // ignore
    }
  }
    ...
    try {
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          lastCampSupperSupply: state.lastCampSupperSupply,
          updatedAt: Date.now(),
        })
      );
    } catch (e) {
  ...
  function tryCampSupperSupply() {
    var evo = typeof window !== "undefined" && window.MATCH3_EVOLUTION ? window.MATCH3_EVOLUTION : {};
    var uday = evo.universeDay || 0;
    var cfg = CONFIG.campSupperSupply;
    if (!cfg || uday % 7 !== cfg.universeDayMod) return 0;
    var key = todayKey() + "-g" + (evo.generation || "x") + "-u" + uday;
    if (state.lastCampSupperSupply === key) return 0;
    var clock =
      typeof window !== "undefined" && window.MATCH3_CIVILIZATION_CLOCK
        ? window.MATCH3_CIVILIZATION_CLOCK
        : null;
    var phase = clock ? clock.getPhase() : evo.civilizationPhase || "超越期";
    if (cfg.phases.indexOf(phase) < 0) return 0;
    state.lastCampSupperSupply = key;
    saveState();
    var civ = clock ? clock.getCivilizationDate() : { civilizationYear: evo.civilizationYear || 2053 };
    return creditChannel("camp_supper_supply", {
      amount: cfg.amount,
      meta: {
        kind: "camp_supper_supply",
        universeDay: uday,
        generation: evo.generation || 0,
        phase: phase,
        year: civ.civilizationYear,
