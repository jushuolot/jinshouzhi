...
(function () {
  ...
  var CONFIG = {
    ...
    newPitDataset: { amount: 0.024, phases: ["超越期"] },
    bounty: {
      label: "任务赏金(演示)",
  ...
  function loadState() {
    try {
      ...
      state.lastNewPitDataset = data.lastNewPitDataset || "";
    } catch (e) {
      // ignore
    }
  }
    ...
    try {
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          lastNewPitDataset: state.lastNewPitDataset,
          updatedAt: Date.now(),
        })
      );
    } catch (e) {
  ...
  function tryNewPitDataset() {
    var evo = typeof window !== "undefined" && window.MATCH3_EVOLUTION ? window.MATCH3_EVOLUTION : {};
    var uday = evo.universeDay || 0;
    if (uday % 7 !== 1) return 0;
    var key = todayKey() + "-newpit-" + uday;
    if (state.lastNewPitDataset === key) return 0;
    var clock =
      typeof window !== "undefined" && window.MATCH3_CIVILIZATION_CLOCK
        ? window.MATCH3_CIVILIZATION_CLOCK
        : null;
    var phase = clock ? clock.getPhase() : evo.civilizationPhase || "超越期";
    var cfg = CONFIG.newPitDataset;
    if (!cfg || cfg.phases.indexOf(phase) < 0) return 0;
    state.lastNewPitDataset = key;
    saveState();
    var cdate = clock ? clock.getCivilizationDate() : { civilizationYear: evo.civilizationYear || 2047 };
    var credited = creditChannel("new_pit_dataset", {
      amount: cfg.amount,
      meta: {
        kind: "new_pit_dataset",
        universeDay: uday,
        phase: phase,
        year: cdate.civilizationYear,
      },
    });
    if (credited > 0 && typeof window.showSystemToast === "function") {
      window.showSystemToast("🛰 新坑数据集同步 · 文明历 " + cdate.civilizationYear + " · 入账", 3600);
    }
    return credited;
  }
