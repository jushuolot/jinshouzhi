...
(function () {
  ...
  var CONFIG = {
    ...
    campSupplyLedger: { amount: 0.021, phases: ["超越期"], universeDayMod: 4 },
    bounty: {
      label: "任务赏金(演示)",
  ...
  var registry = {
    ...
    camp_supply_ledger: {
      id: "camp_supply_ledger",
      label: "营地补给账本",
      source: "network_settlement",
      description: "周四生活日整理营地伙食、补给标签与新坑生活档案，模拟结算入账",
    },
  };
  ...
  function loadState() {
    try {
      ...
      state.lastCampSupplyLedger = data.lastCampSupplyLedger || "";
    } catch (e) {
      // ignore
    }
  }
    ...
    try {
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          lastCampSupplyLedger: state.lastCampSupplyLedger,
          updatedAt: Date.now(),
        })
      );
    } catch (e) {
  ...
  function tryCampSupplyLedger() {
    var evo = typeof window !== "undefined" && window.MATCH3_EVOLUTION ? window.MATCH3_EVOLUTION : {};
    var uday = evo.universeDay || 0;
    var cfg = CONFIG.campSupplyLedger;
    if (!cfg || uday % 7 !== cfg.universeDayMod) return 0;
    var key = todayKey() + "-g" + (evo.generation || "x") + "-u" + uday;
    if (state.lastCampSupplyLedger === key) return 0;
    var clock =
      typeof window !== "undefined" && window.MATCH3_CIVILIZATION_CLOCK
        ? window.MATCH3_CIVILIZATION_CLOCK
        : null;
    var phase = clock ? clock.getPhase() : evo.civilizationPhase || "超越期";
    if (cfg.phases.indexOf(phase) < 0) return 0;
    state.lastCampSupplyLedger = key;
    saveState();
    var civ = clock ? clock.getCivilizationDate() : { civilizationYear: evo.civilizationYear || 2053 };
    var credited = creditChannel("camp_supply_ledger", {
      amount: cfg.amount,
      meta: {
        kind: "camp_supply_ledger",
        universeDay: uday,
        generation: evo.generation || 0,
        phase: phase,
        year: civ.civilizationYear,
