...
(function () {
  ...
  var CONFIG = {
    ...
    newPitOpenData: { amount: 0.024, phases: ["复兴期", "超越期"], dayMod: 1 },
    bounty: {
      label: "任务赏金(演示)",
  ...
  var registry = {
    ...
    new_pit_open_data: {
      id: "new_pit_open_data",
      label: "新坑开图档案",
      source: "network_settlement",
      description: "周一新坑开图时同步公开探方线索档案",
    },
  };
  ...
  function loadState() {
    try {
      ...
      state.lastNewPitOpenData = data.lastNewPitOpenData || "";
    } catch (e) {
      // ignore
    }
  }
    ...
    try {
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          lastNewPitOpenData: state.lastNewPitOpenData,
          updatedAt: Date.now(),
        })
      );
    } catch (e) {
  ...
  function tryNewPitOpenData() {
    var evo = typeof window !== "undefined" && window.MATCH3_EVOLUTION ? window.MATCH3_EVOLUTION : {};
    var uday = evo.universeDay || 0;
    var cfg = CONFIG.newPitOpenData;
    if (!cfg || uday % 7 !== cfg.dayMod) return 0;
    var clock =
      typeof window !== "undefined" && window.MATCH3_CIVILIZATION_CLOCK
        ? window.MATCH3_CIVILIZATION_CLOCK
        : null;
    var phase = clock ? clock.getPhase() : evo.civilizationPhase || "复兴期";
    if (cfg.phases.indexOf(phase) < 0) return 0;
    var key = todayKey() + "-newpit-u" + uday;
    if (state.lastNewPitOpenData === key) return 0;
    state.lastNewPitOpenData = key;
    saveState();
    var year = clock ? clock.getCivilizationDate().civilizationYear : evo.civilizationYear || 0;
    var credited = creditChannel("new_pit_open_data", {
      amount: cfg.amount,
      meta: { kind: "new_pit_open_data", universeDay: uday, phase: phase, year: year },
    });
    if (credited > 0 && typeof window.showSystemToast === "function") {
      window.showSystemToast("🧭 新坑开图档案 · 文明历 " + year + " 已同步", 3400);
    }
    return credited;
  }
  ...
  function tryWeeklyRecap() {
    ...
    if (credited > 0 && typeof window.showSystemToast === "function") {
      window.showSystemToast("📋 宇宙第" + uday + "日收工复盘完成", 3600);
    }
    return credited;
  }
