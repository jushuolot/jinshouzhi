...
(function () {
  ...
  var CONFIG = {
    ...
    campProvisionLedger: { amount: 0.021, phases: ["超越期"], universeDayMod: 4 },
    bounty: {
      label: "任务赏金(演示)",
  ...
  var registry = {
    ...
    camp_provision_ledger: {
      id: "camp_provision_ledger",
      label: "营地补给账本",
      source: "network_settlement",
      description: "周四馋嘴生活日整理补给账本、炊烟采样与营地生活层位，模拟档案入账",
    },
  };
  ...
  function loadState() {
    try {
      ...
      state.lastCampProvisionLedger = data.lastCampProvisionLedger || "";
    } catch (e) {
      // ignore
    }
  }
    ...
    try {
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          lastCampProvisionLedger: state.lastCampProvisionLedger,
          updatedAt: Date.now(),
        })
      );
    } catch (e) {
  ...
  function tryCampProvisionLedger() {
    var evo = typeof window !== "undefined" && window.MATCH3_EVOLUTION ? window.MATCH3_EVOLUTION : {};
    var uday = evo.universeDay || 0;
    var cfg = CONFIG.campProvisionLedger;
    if (!cfg || uday % 7 !== cfg.universeDayMod) return 0;
    var key = todayKey() + "-g" + (evo.generation || "x") + "-u" + uday;
    if (state.lastCampProvisionLedger === key) return 0;
    var clock =
      typeof window !== "undefined" && window.MATCH3_CIVILIZATION_CLOCK
        ? window.MATCH3_CIVILIZATION_CLOCK
        : null;
    var phase = clock ? clock.getPhase() : evo.civilizationPhase || "超越期";
    if (cfg.phases.indexOf(phase) < 0) return 0;
    state.lastCampProvisionLedger = key;
    saveState();
    var civ = clock ? clock.getCivilizationDate() : { civilizationYear: evo.civilizationYear || 2053 };
    var credited = creditChannel("camp_provision_ledger", {
      amount: cfg.amount,
      meta: {
        kind: "camp_provision_ledger",
        universeDay: uday,
        generation: evo.generation || 0,
        phase: phase,
        year: civ.civilizationYear,
    ...
    if (credited > 0 && typeof window.showSystemToast === "function") {
      window.showSystemToast("🍲 营地补给账本 · 文明历 " + civ.civilizationYear + " · 入账", 3600);
    }
    return credited;
  }
