...
| `public_commons_mirror` | `network_settlement` | 每日首次启动（超越期公开档案镜像） |
| `beyond_phase_presence` | `passive_yield` | 超越期会话周期微量 tick |
| `weekly_recap` | `network_settlement` | 宇宙第七日收工复盘 |
| `new_pit_signal` | `network_settlement` | 下新坑日同步探针线索 |
...
- 文明时钟已上线；堪舆图、桌面 VN、发现弹窗、三星盛典、新坑线索结算已就绪
